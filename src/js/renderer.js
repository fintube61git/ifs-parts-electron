...
const url = require('url');                 // <-- for safe file:// URLs
...
async function loadFilesOnStartup() {
  // In this app, __dirname resolves to the folder containing index.html: .../src
  // So assets live at: .../src/images and .../src/data/questions.json
...
function showImage() {
  ...
  // Use a Windows-safe, cross-platform file URL
  canvas.src = url.pathToFileURL(imageFile.path).href;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('exportHtml').addEventListener('click', () => {
    ...
    try {
      ...
      .report-card-responses .optionGroup input { display: none; }
      .report-card-responses .optionGroup label { flex: 1 1 auto; display: block; padding: 0.5rem 1rem; text-align: center; border: 1px solid var(--border-color); border-radius: 20px; cursor: pointer; transition: background-color 0.2s ease, border-color 0.2s ease; }
